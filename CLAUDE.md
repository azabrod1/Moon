...
- Startup URL params: `?auto=volumeCompare` boots the "How many fit?" compare mode; `?debug=1` shows the error/debug overlay; `?nosw=1` unregisters the service worker and deletes its caches (the kill switch); `?sectors=0` turns off surface sector streaming (Earth/Moon/Mars draw their equirect maps only — the A/B for tile questions); `?msaa=0` turns the composer scene target's multisampling off and brings back the old 1.5× supersample floor, i.e. production's rendering before multisampling (the kill switch; policy in `src/app/renderResolution.ts`); the dev server also takes `?msaa=2|4|8` to force a count on any display (the A/B for antialiasing questions — 4 or 8 on a dense display can exhaust GPU memory, so production ignores them).
