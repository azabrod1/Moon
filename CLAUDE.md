...
Headless screenshots: `node tools/shoot.mjs` drives the same bridge; the method, the real-GPU flags, and the before/after recipe are in the `headless-webgl-screenshots` skill. `tools/` holds the wider capture/forensics kit — look there before writing a new one. The asserting batteries are `tools/oval-probe.mjs` (the lens), `tools/flyby-probe.mjs` (arrivals), `tools/smoothness-gate.mjs` (frame delivery) and `tools/sector-probe.mjs` (the sector tiles and the globe texture ladder against the memory envelope they share — `--scenario=budget,touch,tour,moon-tour,envelope,gpu,sweep`, and `--tiles=` for a level the app does not ship). The last two take `/tmp/moon-browser.lock`, so no two browser runs on this machine can share a GPU. Ad-hoc Playwright scripts live inside the repo (`tools/` or `planning/`), never `/tmp` (ESM resolves `node_modules` from the script's location).
...
- Startup URL params: `?auto=volumeCompare` boots the "How many fit?" compare mode; `?debug=1` shows the error/debug overlay; `?nosw=1` unregisters the service worker and deletes its caches (the kill switch); `?sectors=0` turns off surface sector streaming (Earth/Moon/Mars draw their equirect maps only — the A/B for tile questions); `?synth=0` holds the close-range detail synthesis at zero on every surface (the A/B for what a magnified surface looks like without it); `?tilebytes=0` uploads every sector tile straight from its ImageBitmap instead of decoding it to raw bytes first (the A/B for tile-UPLOAD questions — the byte path's whole point is that the driver has no source to convert; it works in prod like `?sectors=0`, because the stutter it addresses was reported from a phone, and `?debug=1`'s memory line plus `__moon.sectors().tilePixels` say which path a device is actually taking); `?tiles=<url prefix>` (DEV only) fetches sector tiles from somewhere else — rarely needed now that `npm run dev` also serves the staged levels in the main checkout's `.moon-data-cache/tiles-staging` on its own origin (`tools/devTilesPlugin.mjs`; `MOON_TILES_ROOT` moves the root), so a plain dev URL streams every level; `?envelope=<MiB>` (DEV only) shrinks the shared GPU memory envelope, which is how a desktop is put under a phone's pressure without a phone (the profile's ceiling and sector floor stay put).
