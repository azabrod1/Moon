...
- Startup URL params: `?auto=volumeCompare` boots the "How many fit?" compare mode; `?debug=1` shows the error/debug overlay; `?nosw=1` unregisters the service worker and deletes its caches (the kill switch); `?sectors=0` turns off surface sector streaming (Earth/Moon/Mars draw their equirect maps only — the A/B for tile questions); `?synth=0` holds the close-range detail synthesis at zero on every surface (the A/B for what a magnified surface looks like without it); `?tiles=<url prefix>` (DEV only) fetches sector tiles from somewhere else — rarely needed now that `npm run dev` also serves the staged levels in the main checkout's `.moon-data-cache/tiles-staging` on its own origin (`tools/devTilesPlugin.mjs`; `MOON_TILES_ROOT` moves the root), so a plain dev URL streams every level; `?envelope=<MiB>` (DEV only) shrinks the shared GPU memory envelope, which is how a desktop is put under a phone's pressure without a phone (the profile's ceiling and sector floor stay put).
