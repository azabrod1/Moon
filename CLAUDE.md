...
Generated / transcribed sources: `src/astronomy/satelliteElements.ts` is generated — edit only via `npm run gen:moons`; `standish.ts` element values are transcribed verbatim — never re-round them.
...
- Startup URL params: `?auto=volumeCompare` boots the "How many fit?" compare mode; `?debug=1` shows the error/debug overlay.
