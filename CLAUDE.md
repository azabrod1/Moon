...
- **Sector tile sets are addressed by content**: `textures/tiles/<key>/<tier>.<setHash8>/<c>_<r>.webp`, the hash covering the whole set's bytes, written by `tools/gen-tiles.mjs` into `sectorSets.generated.ts` (the app's hashes) and `tiles/sets.json`. A re-cut set lands on a path nothing has cached, so the worker keeps such tiles with no digest and no expiry and prunes the sets the app stops naming. `VITE_TILE_ORIGIN` (empty = same origin) moves tiles to another host; it is read in `texturePolicy.ts` and by `swPlugin` for the worker's allowlist — one variable, or the two would drift. Tiles fail open to the base map, so failures are made loud on purpose: the build throws when sets are named and neither the origin nor `dist/textures/tiles` exists, and dev logs the first tile that doesn't load.
