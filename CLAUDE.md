...
- Startup URL params: `?auto=volumeCompare` boots the "How many fit?" compare mode; `?debug=1` shows the error/debug overlay; `?nosw=1` unregisters the service worker and deletes its caches (the kill switch); `?sectors=0` turns off surface sector streaming (Earth/Moon/Mars draw their equirect maps only — the A/B for tile questions); `?msaa=0|2|4|8` forces the composer scene target's sample count (the A/B for antialiasing questions; policy in `src/app/renderResolution.ts`; a diagnostic, not a setting — 4 or 8 on a dense display can exhaust GPU memory).
