/**
 * Draws the dashed orbit-details overlay around the Moon in the Moon view —
 * major axis, minor axis, perigee/apogee markers, and the Sun-pointing line —
 * to make orbital geometry (eccentricity, inclination) legible at a glance.
 */
...
export class OrbitDetailsOverlay {
  ...
  constructor() {
    ...
    const earthFocus = createFocusRing(SCENE_UNITS.EARTH_RADIUS * 1.18, 0, 0xffdf8c, 0.42);
    const secondFocus = createFocusRing(SCENE_UNITS.EARTH_RADIUS * 0.82, -focalOffsetScene * 2, 0xa8d4ff, 0.38);
    const centerRing = createFocusRing(SCENE_UNITS.EARTH_RADIUS * 0.38, -focalOffsetScene, 0x8eb7ff, 0.18);
  ...
  update(nodeAngleDeg: number, moonMeanAnomalyDeg: number) {
    ...
    orientOrbitPlane(this.group, SCENE_UNITS.MOON_INCLINATION, nodeAngleDeg * DEG2RAD);
