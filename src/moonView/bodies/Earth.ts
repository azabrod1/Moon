/**
 * Earth for the Moon view. Owns the day-side mesh, cloud shell, atmosphere
 * rim glow, and the sun-direction-driven night lights shader. Units are scene
 * units (1 = Earth radius), not AU — the Moon view uses an artistically
 * compressed geometry so the Moon and shadow cone stay readable.
 */
...
export class Earth {
  ...
  constructor(textures: LoadedTextures) {
    ...
    // Earth axial tilt
    this.group.rotation.z = SCENE_UNITS.EARTH_AXIAL_TILT;

    // Main Earth mesh
    const earthGeo = new THREE.SphereGeometry(SCENE_UNITS.EARTH_RADIUS, 128, 64);
    ...
    const nightGeo = new THREE.SphereGeometry(SCENE_UNITS.EARTH_RADIUS * 1.001, 128, 64);
    ...
    // Clouds
    const cloudsGeo = new THREE.SphereGeometry(SCENE_UNITS.EARTH_RADIUS * 1.01, 128, 64);
    ...
    // Atmosphere glow
    const atmosGeo = new THREE.SphereGeometry(SCENE_UNITS.EARTH_RADIUS * 1.06, 64, 32);
