/**
 * Sun for the Moon view. Animated-corona shader sphere, two billboard glows,
 * and a shadow-casting directional light aimed at Earth. Distance to Earth
 * is artistically compressed (SCENE_UNITS.EARTH_SUN_DIST), not real AU.
 */
...
export class Sun {
  ...
  constructor(useBloom = true) {
    ...
    this.group.position.set(SCENE_UNITS.EARTH_SUN_DIST, 0, 0);

    // Sun surface with animated corona shader
    const sunGeo = new THREE.SphereGeometry(SCENE_UNITS.SUN_RADIUS, 64, 32);
    ...
    this.light.shadow.camera.far = SCENE_UNITS.EARTH_SUN_DIST * 2;
    const shadowSize = SCENE_UNITS.EARTH_MOON_DIST * 1.5;
    ...
    this.light.target.position.set(-SCENE_UNITS.EARTH_SUN_DIST, 0, 0);
  ...
  setPosition(angleDeg: number) {
    ...
    this.group.position.set(
      SCENE_UNITS.EARTH_SUN_DIST * Math.cos(angle),
      0,
      SCENE_UNITS.EARTH_SUN_DIST * Math.sin(angle),
    );
