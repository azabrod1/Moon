/**
 * Builds and updates the dashed Moon orbit line shown around Earth in the
 * Moon view. Excludes a short arc around the Moon's current position so the
 * line doesn't visibly pierce the sphere.
 */
import * as THREE from 'three';
import { SCENE_UNITS } from '../shared/constants/sceneUnits';
import { createOrbitPoints } from '../astronomy/lunarOrbit';
...
export function updateMoonOrbitLine(
  ...
  const geometry = buildOrbitLineGeometry(
    MOON_ORBIT_SEGMENTS,
    moonLocalPosition,
    SCENE_UNITS.MOON_RADIUS * moonRadiusScale * 1.25,
  );
