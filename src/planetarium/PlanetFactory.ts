 ...
 * measured surface-view stall). These tiny meshes carry exactly the
 ...
 * Add to the scene before renderer.compileAsync and KEEP them there for the
 * session: three destroys a program the moment its last material is disposed,
 * and a combination no live material holds yet at boot (a measured normal
 * before the Moon's arrives, the cloud deck's relief) has the probe as its only
 * holder — disposing the probes after the compile threw those programs away,
 * and the first real draw linked them again mid-flight. The group is
 * invisible for ordinary frames and never raycast; activation briefly makes
 * it visible only for a one-pixel, load-veiled real draw on drivers where
