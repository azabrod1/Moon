...
export class PlanetariumMode {
  ...
  private enterSystemMap(): void {
    ...
    // The map's early return skips the pipelines that reposition these 3D
    // layers, so hide them (else they ghost at stale floating-origin coords).
    ...
    // The starfield keeps its sun-glare mask (dims stars around the Sun's screen
    // spot). Nothing re-drives it in the map, so clear it — otherwise a stale
    // dark hole from the pre-map view sits in the starfield. The normal pipeline
    // re-activates it on exit.
    if (this.starfield) {
  ...
  private rebuildOrbitLinesIfStale() {
    ...
    // positions around the epoch — past the Standish validity (~3000 AD/BC) they
    // all clamp to the boundary and the ellipse collapses to a point. Clamp the
    // sampling epoch (with margin for the longest period) so orbits stay drawn
    // at extreme dates; the clamped planets still sit on the clamped ellipse.
    const YEAR_MS = 365.25 * 86_400_000;
    const J2000_MS = Date.UTC(2000, 0, 1);
    const epoch = Math.max(J2000_MS - 4750 * YEAR_MS, Math.min(J2000_MS + 750 * YEAR_MS, this.timeState.currentUtcMs));
    if (Math.abs(epoch - this.solarSystem.orbitLinesEpochUtcMs) < SIXTY_DAYS_MS) {
      return;
    }
