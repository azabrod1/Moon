...
describe('clock sanitizing survives the first running frame', () => {
  it('keeps a save at the Date bound valid after one unpaused frame, and clamps a wild rate to the ladder', async () => {
    const { advancePlanetariumTime } = await import('../astronomy/planetary');
    const { MAX_UTC_MS } = await import('../astronomy/constants');
    ...
    // The rail's top preset, as a literal so a ladder change is a deliberate edit here too.
    expect(Math.abs(state.astroTimeRate!)).toBeLessThanOrEqual(31_557_600);
