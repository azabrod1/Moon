...
export class SystemMap {
  ...
  private readonly moonHoverTargets: Array<{ name: string; parent: string; color: number; radiusAU: number; x: number; y: number; z: number }> = [];
  ...
  private moonMapPosition(moonName: string, parentName: string, out: THREE.Vector3): boolean {
    ...
    if (realDist < 1e-12) return false;
    const orbit = systemMapMoonOrbitRadius(realDist / planet.data.radiusAU, systemMapBodyRadius(planet.data.radiusAU));
    out.copy(planet.group.position).addScaledVector(off, orbit / realDist);
    return true;
  }
  ...
  resetView(): void {
    ...
    const pos = this.tmpA.set(this.frameExtent * 0.05, this.frameExtent * 0.62, this.frameExtent * 1.55);
    this.startTransition(target, pos);
    this.updateFocusUI();
  }
  ...
  private updateMoons(utcMs: number): void {
    ...
    for (const planet of this.objects.planets) {
      ...
      for (const m of moons) {
        const off = computeMoonOffsetEquatorialAU(m.data.name, planet.data.name, utcMs, this.tmpMoon);
        const realDist = off.length();
        if (realDist < 1e-12) { m.mesh.visible = false; continue; }
        m.mesh.position.copy(off).multiplyScalar(systemMapMoonOrbitRadius(realDist / planet.data.radiusAU, parentMapRadius) / realDist);
        ...
        this.moonHoverTargets.push({ name: m.data.name, parent: planet.data.name, color: m.data.color, radiusAU: m.data.radiusAU, x: wx, y: wy, z: wz });
      }
    }
  }
  ...
  private frameAll(): void {
    ...
    this.camera.position.set(extent * 0.05, extent * 0.62, extent * 1.55);
    this.controls.target.set(0, 0, 0);
    this.controls.enabled = true;
    this.controls.minDistance = extent * 0.06;
    this.controls.maxDistance = extent * 3.4;
  ...
  private buildLabels(): void {
    ...
    if (!container) return;
    const add = (name: string, color: number, desc: string, rows: Array<[string, string]>): void => {
    ...
    for (const b of PLANETARIUM_BODIES) {
      add(b.name, b.color, b.description, [
        ['Distance', fmtAu(b.semiMajorAxisAU)],
    ...
    this.moonLabel = this.makeLabelEl('', 0x8fdcff);
  ...
  /** Replace a label's fact rows with `[key, value]` pairs. */
  private setLabelRows(label: HTMLElement, rows: Array<[string, string]>): void {
  ...
  private renderLabels(): void {
    ...
    let bestX = 0;
    let bestY = 0;
    let bestMoon: { color: number; radiusAU: number; name: string; parent: string } | null = null;
  ...
  private readonly onPointerUp = (e: PointerEvent): void => {
    ...
    const rect = this.renderer.domElement.getBoundingClientRect();
    const name = this.pickBodyAt(e.clientX - rect.left, e.clientY - rect.top);
    if (name) this.focusOn(name); // click empty space: keep the current view
  };
  ...
  private pickBodyAt(px: number, py: number): string | null {
    const width = this.renderer.domElement.clientWidth;
    const height = this.renderer.domElement.clientHeight;
    ...
    for (const [name, world] of this.labelWorld) {
      ...
      if (d < hit && d < bestDist) { bestDist = d; best = name; }
    }
    return best;
  }
}
