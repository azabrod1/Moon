 ...
 *  b = m·d/√(d²−m²). The standoff keeps d well above m in the whole
 *  catalog, but that is a property of the catalog, not of the construction:
 *  a park pulled inside the miss would make the root imaginary, so that case
 *  aims straight at the body, which is what a park that close already looks
 *  like. */
function impactParameterAU(inp: ArrivalInputs, dist: number): number {
  const collisionR = moonCollisionRadius(inp.renderedR, inp.shipClearance);
  const missM = Math.max(collisionR * 1.15, CRUISE_CAM_DIST_AU);
  if (dist <= missM) return 0;
