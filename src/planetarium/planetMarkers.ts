...
export interface PlanetMarkerVisual {
  /** Multiplier on PlanetLabels' viewport-pinned base scale. Always in
   *  [sizeMinScale, 1], so photometry can shrink a fainter marker but never
   *  balloon one past the pin. */
  sizeMul: number;
  /** Material-color multiplier; Sun glare remains a separate opacity factor. */
  brightness: number;
}
