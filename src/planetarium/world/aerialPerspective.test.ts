...
describe('the injected surface shader', () => {
  ...
  it('is the text the goldens were captured through', () => {
    ...
    expect(hash(shader.fragmentShader))
      .toBe('70aecaa0ce3fb99b7430948c070c7289e8cf00aaaf5e8e12da982f2cc3079501');
  });
...
describe('the eclipse on the ground and on the air', () => {
  ...
  it('lets a moon whose umbra only reaches near perigee be a caster at all', () => {
    ...
    // solar eclipse IS. So the candidate set tests the umbra at a perigee
    // margin below the mean (world/moonShadowCasters.ts, CASTER_PERIGEE_MARGIN)
    // rather than at the mean itself, or the one eclipse anybody would go and
    // look at casts no shadow on the ground and none on the air in front of it.
    ...
    expect(reachLimitKm).toBeGreaterThan(384_400 * CASTER_PERIGEE_MARGIN); // and inside the margin
    const mode = src('../PlanetariumMode.ts');
    const casterBlock = mode.slice(
      mode.indexOf('casterCache = {'),
      mode.indexOf('this.moonShadowCasterCache.set'),
    );
    expect(casterBlock).not.toBe('');
    expect(casterBlock).toContain('selectMoonShadowCasters(');
    expect(casterBlock).not.toMatch(/\.filter\(/);
    // And the live per-frame check, which is what actually admits one, still
    // measures the distance the moon is at right now.
    expect(mode).toContain('&& umbraReachesSurface(m.data.radiusAU, offset.length(), sunTanAtParent)');
  });
